...
import { introspect, buildGraph, analyzeSchema, registerIntrospector, printCoverageTable } from '@seedforge/core';
import type { DatabaseSchema, FieldSemanticMatch, SeedForgeConfig } from '@seedforge/core';
...
export async function initCommand(opts: { config?: string; force?: boolean }): Promise<void> {
  ...
  for (const table of schema.tables) {
    ...
    for (const m of tableResolved) {
      const paramsStr = JSON.stringify(m.suggestedGenerator.params);
      fields[m.column] = {
  ...
  const configObj: SeedForgeConfig = {
    ...
    tables: tables as Record<string, SeedForgeConfig['tables'][string]>,
  };

  const configContent = generateConfigFile(configObj, unresolved as FieldSemanticMatch[]);
...
function generateConfigFile(config: SeedForgeConfig, unresolved: FieldSemanticMatch[]): string {
  const lines: string[] = [];

  lines.push('// Auto-generated by `seedforge init`');
  lines.push('// Review and customize before running `seedforge seed`');
  lines.push('');
  lines.push("import { defineConfig } from '@seedforge/core';");
  lines.push('');
